 ...
 * @private
 */
DataAccessObject._coerceProperty = function(p, where, options) {
  const self = this;
  ...
  if (!DataType) {
    if (p.indexOf('.') !== -1) {
      DataType = this._coerceNestedProperty(p, props);
    } else {
      return;
    }
  }
...
DataAccessObject._coerceNestedProperty = function(p, props) {
  const parts = p.split('.');
  const end = parts.pop();
  var obj = props;
  while (parts.length) {
